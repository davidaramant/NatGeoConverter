...
namespace DataModelLoader {
    class MainClass {
        ...
        private static void LoadArticles( IProjectConfig config ) {
            using( var db = new SQLiteConnection( databasePath: config.DatabasePath ) ) {
                db.DropTable<Article>();
                db.CreateTable<Article>();
        
                var maxDate = db.Table<Issue>().Max( i => i.ReleaseDate );
                var maxSearchTime = maxDate.ToSearchTime();

                var cngDbPath = Path.Combine( config.DatabaseDir, "cngcontent.sqlite3" );
                using( var cngDb = new SQLiteConnection( databasePath: cngDbPath ) ) {
        ...
        private static int GetIssueIdFromSearchTime( SQLiteConnection db, int searchTime ) {
            var date = new DateTime( searchTime / 10000, (searchTime / 100) % 100, searchTime % 100 );

            var issue = db.Table<Issue>().Where( i => i.ReleaseDate == date ).FirstOrDefault();
                
            if( issue == null ) {
                throw new ArgumentException( "Could not find issue for: " + searchTime );
            }

            return issue.Id;
        }

        private static int GetPageId( SQLiteConnection db, int issueId, int offset ) {
            ...
            using( var db = new SQLiteConnection( dbPath ) ) {
                ...
                foreach( var op in Enum.GetValues( typeof( CorrectionOperation)).Cast<CorrectionOperation>() ) {
                    ...
                    foreach( var i in issuesWithOp ) {
                        ...
                        Out.WL(
                            "E: {0}",
                            JsonUtility.FormatJson( NGModelExtensions.ConvertToJson( i.Issue.page_exceptions ) ) );
                        Out.WL();
                    }
                }
            }
            ...
            using( var db = new SQLiteConnection( databasePath: config.DatabasePath ) ) {
                ...
                foreach( var decadeDir in Directory.GetDirectories( config.AbsoluteFullImageDir ) ) {
                    ...
                    foreach( var issueDir in Directory.GetDirectories( decadeDir ) ) {
                        ...
                        var issue = new Issue {
                            DecadeId = decade.Id,
                            ReleaseDate = ParseIssueDirIntoDate( issueDirName )
                        };
                        db.Insert( issue );
